...
Another important role for the recommended `readme.txt` file is to be the place where the AI researcher who is sharing the project specifies precisely the **starting point** for the annotation customisation/enhancement project.  The **default starting point** for many (even most) NeSy4VRD annotation customisation/enhancement projects will be the NeSy4VRD visual relationship annotations distributed in the **NeSy4VRD dataset package** published on Zenodo. But even here the possibility exists for multiple versions of the NeSy4VRD visual relationship annotations to emerge over time and be published on Zenodo under different version numbers. So even a **default starting point** should specify the **version number** that NeSy4VRD dataset package on Zenodo **explicitly**.

Interestingly, however, the project **reuse** and **composition** capabilities of the **Distributed Annotation Enhancement** model of collaborative data annotation enabled by NeSy4VRD mean that the **starting point** of a NeSy4VRD annotation customisation/enhancement project is not confined to being the default NeSy4VRD visual relationship annotations distributed on Zenodo.  In fact, the **starting point** for a NeSy4VRD annotation customisation/enhancement project can, in theory, be fairly arbitrary. Several possibilities exist.

Here are the categories covering possible NeSy4VRD annotation customisation/enhancement project **starting points**:
* *(i)* the **default** NeSy4VRD visual relationship annotations published on Zenodo;
* *(ii)* the NeSy4VRD annotations **reused** from some other **shared** NeSy4VRD project;
* *(iii)* the **default** NeSy4VRD annotations **composed** with the customisations/enhancements of multiple other **shared** NeSy4VRD projects;
* *(iv)* the NeSy4VRD annotations **reused** from some other **shared** NeSy4VRD project **composed** with the customisations/enhancements of multiple other **shared** NeSy4VRD projects.

Given this range of possibilities for the **starting point** of a NeSy4VRD annotation customisation/enhancement project, one can see why it is important to **explicitly specify the starting point** of a project in the `readme.txt` file of the `nesy4vrd_customisation_project` component of the shared project package. An AI researcher seeking to **reuse** and, especially, **compose** the **shared** project is likely to find this information essential.

Let us consider a simple example to illustrate these points. Suppose we use $N$ to denote the default NeSy4VRD visual relationship annotations published on Zenodo. And suppose we use $A$ and $B$ to denote two **shared** projects that have been **composed** with $N$ to establish the **starting point** for a new annotation customisation/enhancement project that we denote by $C$. In this case, the **starting point** for project $C$ can be described as
$$N + A + B.$$ The project files packaged in directory `nesy4vrd_customisation_project` for shared project $C$ need be only the files specific to project $C$, because the files for projects $A$ and $B$, if ever needed, can be obtained online from their **shared** NeSy4VRD customisation project packages.

It is also possible that because project $C$ was built upon annotations $N+A+B$ there are some subtle dependencies between some of the annotation customisations specified for $C$ and the annotations $N+A+B$. If so, then if project $C$ is **composed** with annotations other than $N+A+B$, these dependencies may potentially lead to **incompatibilities** being detected by the protocol driver of the **NeSy4VRD workflow** when project $C$ is **composed** with other sets of annotations. This possibility of **incompatibilities** arising is another reason why **explicitly specifying the starting point** of a NeSy4VRD project is **good practice**. An AI researcher **sharing** a project should wish to fully disclose all relevant information to AI researchers who might opt to **compose** their **shared** project with other projects. Detailed information about potential **incompatibilities** that can arise during project **composition**, and how to resolve them, is provided in the companion document `README_UsingSharedCustomisationProjects.md`.
