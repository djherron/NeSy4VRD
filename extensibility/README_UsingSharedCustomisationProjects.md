...
If an AI researcher wishes to **reuse** an individual shared NeSy4VRD annotation customisation/enhancement project **as is** and **in isolation**, the simplest and most direct method for doing so is to  **reuse** the prepared annotations files provided in the `nesy4vrd_annotations` component (folder) of the customisation project package.

If the AI researcher who is **reusing** the shared project is doing NeSy research using OWL ontologies or OWL-based knowledge graphs, the `nesy4vrd_ontology` component of the project package also comes into play. Not all AI researchers are familiar with or interested in the Semantic Web and OWL ontologies. So the VRD-World OWL ontology provided in the `nesy4vrd_ontology` component of the shared project package may or may not be aligned with the annotations provided in the `nesy4vrd_annotations` component of the project package. This depends entirely on the AI researcher who **undertook** and **shared** the NeSy4VRD project. Information about alignment or non-alignment may be provided in the `readme.txt` file of the `nesy4vrd_ontology` component, but this again depends on AI researcher who **undertook** and **shared** the project.  The AI researcher who is **reusing** the shared project may need to investigate the question of ontology alignment or non-alignment. If the ontology is not aligned with the annotations, the AI researcher who is **reusing** the shared NeSy4VRD project will need to customise the VRD-World ontology manually in order to re-establish full alignment.

It is also possible to **reuse** a shared NeSy4VRD project by using the the information and project files provided in the `nesy4vrd_customisation_project` component (folder) of the customisation project package to **re-run** the application of the project's customisations/enhancements using the **NeSy4VRD workflow** infrastructure. As long as the **starting point** of the shared project is first correctly established (as described in the `readme.txt` file in the `nesy4vrd_customisation_project` component), this approach will recreate exact versions of the annotations files provided in the `nesy4vrd_annotations` component. But there is no point in taking this approach if the AI researcher intends only to **reuse** the individual shared NeSy4VRD annotation customisation/enhancement project **as is** and **in isolation**. It is simpler to use the prepared annotations files provided in the `nesy4vrd_annotations` component rather than recreate copies of them.

If an AI researcher is looking to **compose** the shared NeSy4VRD project's customisations/enhancements with their own work and/or with other shared NeSy4VRD projects, then that researcher must use the project files provided in the `nesy4vrd_customisation_project` component (folder) of the customisation project package to **re-run** the application of the project's customisations/enhancements using the **NeSy4VRD workflow** infrastructure. A full discussion of **composition** is provided in the following section.


## Composing annotations of multiple NeSy4VRD customisation projects

Being able to **reuse** a shared NeSy4VRD annotation customisation/enhancement project by freely **composing** it with other shared projects, in unrestricted and arbitrary ways, is a key aspect of the value of the **Distributed Annotation Enhancement** (**DAE**) model of collaborative data annotation enabled by the **NeSy4VRD extensibility support infrastructure**. This ability to **reuse** multiple shared NeSy4VRD projects by freely **composing** them into unique permutations and combinations requires that AI researchers work with the project files provided in the `nesy4vrd_customisation_project` component of NeSy4VRD customisation project packages. This is the key reason why the `nesy4vrd_customisation_project` component is included in the customisation project package: to enable project **composition**.

The **NeSy4VRD workflow** configuration module files, and the **NeSy4VRD protocol** annotation customisation instruction text files, provided in the `nesy4vrd_customisation_project` component allow anyone to **replay** or **rerun** the NeSy4VRD annotation customisation/enhancement project that has been shared, and to do so by **layering** those annotation customisations/enhancements upon any set of NeSy4VRD annotations, however established (perhaps by having previously **composed** several other shared projects). 
...
## Composing OWL ontologies of multiple NeSy4VRD customisation projects

In our discussion of **sharing** NeSy4VRD annotation customisation/enhancement projects in the companion document `README_SharingCustomisationProjects.md`, we briefly mention the topic of project **composition** as it relates to the NeSy4VRD OWL ontology, VRD-World. Here we return to this topic in order to discuss it more fully.

Researchers pursuing NeSy AI using OWL ontologies or OWL-based knowledge graphs can **undertake**, **share** and **reuse** VRD-World OWL ontology customisations and enhancements in tandem with visual relationship annotation customisations/enhancements. But NeSy4VRD currently provides infrastructure for **automating** the **composition** of multiple, shared NeSy4VRD customisation/enhancement projects only with respect to the visual relationship **annotations**, not the companion VRD-World **OWL ontologies**.  If an AI researcher doing NeSy AI using OWL ontologies or OWL-based knowledge graphs wishes to **compose** multiple customised/enhanced versions of the VRD-World OWL ontology, this must currently be done **manually**, using an ontology editor such as Protege.

This absence of functionality for automating the **composition** of multiple versions of the VRD-World OWL ontology, however, can be regarded as representing an interesting **research opportunity**. The key to automating the **composition** of the annotation customisations/enhancements of shared NeSy4VRD projects lies in the **NeSy4VRD protocol** and the **NeSy4VRD workflow**, in particular the protocol driver of the **NeSy4VRD workflow**. The **NeSy4VRD protocol** allows annotation customisation/enhancement instructions to be specified declaratively in text files; the protocol driver of the **NeSy4VRD workflow** interprets and executes the instructions. If such functionality could be replicated in the context of OWL ontologies then, conceivably, automated NeSy4VRD project **composition** could be extended to cover both the dataset **annotation** customisations/enhancements and the companion VRD-World **OWL ontology** customisations/enhancements.  Studying how to develop and provide such functionality may well appeal to anyone interested in the problems of automated **ontology alignment** and/or automated **ontology or knowledge graph construction**.
